...
class AssetControllerTest {
    ...
    private final CalculationResult sampleResult = new CalculationResult(
            "VFIAX", "Vanguard 500 Index Fund", AssetType.MUTUAL_FUND,
            10_000, 10, 1.1, 0.0425, 0.1553, 0.16658, 52_345.67
    );

    // --- Response shape tests ---

    @Test
    void getAllAssets_returnsJsonArray() throws Exception {
        ...
        mockMvc.perform(get("/api/assets"))
                ...
                .andExpect(jsonPath("$[0].type").value("MUTUAL_FUND"))
                .andExpect(jsonPath("$[1].ticker").value("SPY"))
                .andExpect(jsonPath("$[1].type").value("ETF"));
    }
    ...
    void calculate_returnsFullResponseShape() throws Exception {
        ...
        mockMvc.perform(get("/api/calculate")
                ...
                .andExpect(jsonPath("$.assetName").value("Vanguard 500 Index Fund"))
    ...
    void calculate_returns404_whenTickerNotFound() throws Exception {
        when(calculationService.calculate(eq("UNKNOWN"), anyDouble(), anyInt()))
                .thenThrow(new IllegalArgumentException("Asset not found: UNKNOWN"));
